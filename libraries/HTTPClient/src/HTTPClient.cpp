    ...
    if(buff) {
        ...
        while(connected() && (stream->available() > -1) && (len > 0 || len == -1)) {
            ...
            if(sizeAvailable) {
                ...
                // read data
                int bytesRead = stream->readBytes(reinterpret_cast<char*>(buff), readBytes);
        ...
        if(size && (int) size != bytesWritten) {
            log_d("Stream payload bytesWritten %d and size %ld mismatch!.", bytesWritten, size);
...
int HTTPClient::writeToStream(Stream * stream)
{
    ...
    if(_transferEncoding == HTTPC_TE_IDENTITY) {
        ...
        while(1) {
            ...
            char buf[2];
            auto trailing_seq_len = _client->readBytes((char*)buf, 2);
    ...
    if(buff) {
		...
		if (readBytes == 0)
			break;
                ...
                // read data
                int bytesRead = _client->readBytes((char*)buff, readBytes);
