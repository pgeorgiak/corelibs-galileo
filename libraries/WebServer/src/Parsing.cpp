...
void WebServer::_parseArguments(String data) {
  ...
  for (iarg = 0; iarg < _currentArgCount;) {
    ...
    printf("pos %d =@%d &@%d\n", pos, equal_sign_index, next_arg_index);
    if ((equal_sign_index == -1) || ((equal_sign_index > next_arg_index) && (next_arg_index != -1))) {
      printf("arg missing value: %d\n", iarg);
      if (next_arg_index == -1)
        break;
    ...
    printf("arg %d key: %s value: %s\n", iarg, arg.key.c_str(), arg.value.c_str());
    ++iarg;
    if (next_arg_index == -1)
      break;
    pos = next_arg_index + 1;
...
bool WebServer::_parseForm(WiFiClient& client, String boundary, uint32_t len){
  (void) len;
  printf("Parse Form: Boundary: %s Length: %d\n", boundary.c_str(), len);
  String line;
  int retry = 0;
  do {
      ...
      if (line.length() > 19 && line.substring(0, 19).equalsIgnoreCase(F("Content-Disposition"))){
        ...
        if (nameStart != -1){
          ...
          if (nameStart == -1){
            ...
            argIsFile = true;
            printf("PostArg FileName: %s\n",argFilename.c_str());
            //use GET to set the filename if uploading using blob
            if (argFilename == F("blob") && hasArg(FPSTR(filename)))
              argFilename = arg(FPSTR(filename));
          }
          printf("PostArg Name: %s\n", argName.c_str());
          using namespace mime;
          ...
          if (!argIsFile){
            ...
            if (line == ("--"+boundary+"--")){
              printf("Done Parsing POST\n");
              break;
            } else if (_postArgsLen >= WEBSERVER_MAX_POST_ARGS) {
              printf("Too many PostArgs (max: %d) in request.\n", WEBSERVER_MAX_POST_ARGS);
              return false;
            }
          } else {
