...
FileImplPtr VFSImpl::open(const char* path, const char* mode)
{
    ...
    if(!path || path[0] != '/') {
        printf("%s does not start with /", path);
        return FileImplPtr();
    }
    ...
    if(!stat(temp, &st)) {
        ...
        printf("%s has wrong mode 0x%08X", path, st.st_mode);
        return FileImplPtr();
    }
...
bool VFSImpl::rename(const char* pathFrom, const char* pathTo)
{
    ...
    if(!exists(pathFrom)) {
        printf("%s does not exists", pathFrom);
        return false;
    }
...
bool VFSImpl::remove(const char* path)
{
    ...
    if(!f || f.isDirectory()) {
        ...
        printf("%s does not exists or is directory", path);
        return false;
    }
    f.close();
...
bool VFSImpl::rmdir(const char *path)
{
    ...
    if (strcmp(_mountpoint, "/spiffs") == 0) {
        printf("rmdir is unnecessary in SPIFFS");
        return false;
    }
    ...
    if(!f || !f.isDirectory()) {
        ...
        printf("%s does not exists or is a file", path);
        return false;
    }
    f.close();
    ...
    , _written(false)
{
    ...
    if(!stat(temp, &_stat)) {
        ...
        if (S_ISREG(_stat.st_mode)) {
            ...
            _f = fopen(temp, mode);
            if(!_f) {
                printf("fopen(%s) failed", temp);
            }
        } else if(S_ISDIR(_stat.st_mode)) {
            _isDirectory = true;
            _d = opendir(temp);
            if(!_d) {
                printf("opendir(%s) failed", temp);
            }
        } else {
            printf("Unknown type 0x%08X for file %s", ((_stat.st_mode)&_IFMT), temp);
        }
    } else {
        //file not found
        if(!mode || mode[0] == 'r') {
